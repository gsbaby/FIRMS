...
def getGeomTables():
    ...
    try:
        results = connectPostGIS('select * from db_connect_table')
        result = []
        for i in results:
            result.append({
                'table_name': i[0],
...
def getGeoJson(table_name):
    ...
    if "modis" in table_name:
        # MODIS
        sql_infomation = "select 'Feature' as type,ST_AsGeoJSON(geom)::json as geometry,json_strip_nulls(row_to_json(row(pid,latitude,longitude,brightness,scan,track,acq_date,acq_time,satellite,confidence,version,bright_t31,frp,daynight)))::json as properties from {} t where ST_Intersects(t.geom,ST_GeomFromText('polygon((73.6667 4.8667,73.6667 53.6167,135.5 53.6167,135.5 4.8667,73.6667 4.8667))',4326))".format(
            table_name)
    else:
        # 葵花
        sql_infomation = "select 'Feature' as type,ST_AsGeoJSON(geom)::json as geometry,json_strip_nulls(row_to_json(row(pid,year,month,day,time,lat,lon,area,volcano,level,reliability,frp,qf,hot)))::json as properties from {} t where ST_Intersects(t.geom,ST_GeomFromText('polygon((73.6667 4.8667,73.6667 53.6167,135.5 53.6167,135.5 4.8667,73.6667 4.8667))',4326))".format(
            table_name)
...
if __name__ == '__main__':
    ...
    sql_infomation = sql_infomation.format('H08_20211027_0620_L2WLF010_FLDK'.lower())
    sql_infomation_all = connectPostGIS(sql_infomation)
    print(sql_infomation_all)
